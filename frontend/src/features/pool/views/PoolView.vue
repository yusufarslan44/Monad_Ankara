...
import { ArrowRight, Clock, Landmark, Lock, PiggyBank, TrendingUp, Wallet } from 'lucide-vue-next';
...
import { poolDepositSchema } from '@/shared/lib/formSchemas';
...
const schema = toTypedSchema(poolDepositSchema);
      ...
      <BaseCard>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="mt-1 font-display text-2xl font-bold text-ink-950">{{ formatMON(data.totalDepositedMON) }} MON</p>
          </div>
        </div>
      </BaseCard>
      <BaseCard>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="mt-1 font-display text-2xl font-bold text-ink-950">{{ formatMON(data.totalBorrowedMON) }} MON</p>
          </div>
        </div>
      </BaseCard>
      <BaseCard>
        <div class="flex items-start gap-3">
          ...
          <div>
            ...
            <p class="mt-1 font-display text-2xl font-bold text-ink-950">{{ formatMON(data.availableLiquidityMON) }} MON</p>
          </div>
        </div>
      </BaseCard>
      <BaseCard>
          ...
          >
            ...
            <div class="mt-4 grid gap-2 sm:grid-cols-3">
              <div>
                ...
                <p class="font-display text-xl font-bold text-ink-950">{{ formatMON(deposit.amountMON) }} MON</p>
              </div>
              <div>
                <p class="text-xs font-semibold uppercase tracking-wider text-ink-700">Tahmini faiz</p>
                <p class="font-display text-xl font-bold text-ink-950">+{{ formatMON(deposit.projectedInterestMON) }} MON</p>
              </div>
              <div>
                <p class="text-xs font-semibold uppercase tracking-wider text-ink-700">Getiri orani</p>
                <p class="font-display text-xl font-bold text-ink-950">%{{ deposit.apyBps / 100 }}</p>
              </div>
            </div>
      ...
      <BaseCard>
        ...
        <form class="mt-5 space-y-5" @submit="onDeposit">
          ...
          <div>
            ...
            <p v-if="errors.lockDays" class="mt-2 text-sm font-medium text-danger-500">{{ errors.lockDays }}</p>
          </div>
          ...
          <div class="rounded-2xl bg-surface-0 p-4">
            ...
            <p class="mt-1 font-display text-3xl font-bold text-ink-950">+{{ formatMON(projectedInterest) }} MON</p>
